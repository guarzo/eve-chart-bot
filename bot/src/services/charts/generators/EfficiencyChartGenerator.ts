...
export class EfficiencyChartGenerator extends BaseChartGenerator {
  ...
  async generateChart(options: {
    ...
    try {
      ...
      const [kills, losses] = await Promise.all([
        errorHandler.withRetry(
          ...
          {
            retries: 3,
            context: { operation: 'fetchKillsForEfficiency', characterCount: characterIds.length, correlationId }
          }
        ),
        errorHandler.withRetry(
          ...
          {
            retries: 3,
            context: { operation: 'fetchLossesForEfficiency', correlationId }
          }
        ),
      ]);
    ...
    const groupData = characterGroups.map(group => {
      ...
      const groupKills = kills.filter(kill => groupCharacterIds.includes(BigIntTransformer.toBigInt(kill.character_id) || BigInt(0)));
      const groupLosses = losses.filter(loss => groupCharacterIds.includes(BigIntTransformer.toBigInt(loss.characterId) || BigInt(0)));
      ...
      throw new ChartError('CHART_GENERATION_FAILED', 'Failed to generate efficiency chart', {
        cause: error,
        context: { correlationId, operation: 'generateEfficiencyChart' }
      });
    }
  }
  ...
  private validateChartOptions(options: {
    ...
    if (issues.length > 0) {
      throw new ValidationError('Invalid chart generation options', issues, { 
        context: { correlationId, operation: 'validateEfficiencyChartOptions' }
      });
    }
  }
}
