...
/**
 * Clean up the character_group table by:
 ...
 */
export async function cleanupCharacterGroups(): Promise<void> {
  const prisma = new PrismaClient();

  try {
    logger.info("Starting character group cleanup process");

    // Check if the CharacterGroupUtils exists
    const utilsPath = path.join(
      __dirname,
      "../../../utils/CharacterGroupUtils.ts"
    );
    let CleanupUtils;

    if (fs.existsSync(utilsPath)) {
      // Use the CharacterGroupUtils if available
      try {
        const {
          CharacterGroupUtils,
        } = require("../../../utils/CharacterGroupUtils");
        CleanupUtils = CharacterGroupUtils;
        logger.info("Using CharacterGroupUtils for cleanup");
      } catch (error) {
        logger.warn(
          "Could not import CharacterGroupUtils, fallback to direct implementation",
          error
        );
        CleanupUtils = null;
      }
    } else {
      logger.info("CharacterGroupUtils not found, using direct implementation");
      CleanupUtils = null;
    }
    ...
    if (CleanupUtils) {
      // Use the utility class if available
      deletedEmptyGroups = await CleanupUtils.cleanupEmptyCharacterGroups(
        prisma
      );
    } else {
      ...
      if (emptyGroups.length > 0) {
        ...
        deletedEmptyGroups = emptyGroups.length;
      }
    }
