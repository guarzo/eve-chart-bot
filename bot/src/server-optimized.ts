...
// Core imports only - delay heavy imports
import express from 'express';
import { logger } from './lib/logger';
import { config as dotenvConfig } from 'dotenv';
import { PrismaClient } from '@prisma/client';

startupProfiler.checkpoint('After core imports');
...
const port = process.env.NODE_ENV === 'test' ? 0 : parseInt(process.env.PORT || '3000', 10);
...
startupProfiler.checkpoint('After basic middleware');
...
async function startServer() {
  ...
  // Start Express server early
  const server = app.listen(port, () => {
    const startupTime = (Date.now() - appStartTime) / 1000;
    logger.info(`Server is running on port ${port} (basic server startup took ${startupTime}s)`);
  });
  ...
  const [
    ...
    { ChartServiceFactory, destroyChartWorkerManager },
    { ChartRenderer },
    { DiscordClient },
    { commands },
    { z },
    ...
    { timerManager },
    { rateLimiterManager },
    { ChartPeriod, ChartSourceType, ChartGroupBy },
    { metricsCollector },
    { tracingService },
    { memoryMonitor },
    { disconnectRedis },
    ...
    import('./infrastructure/repositories/RepositoryManager'),
    import('./services/ingestion/WebSocketIngestionService'),
    import('./services/ingestion/MapActivityService'),
    import('./services/ingestion/CharacterSyncService'),
    import('./services/charts'),
    import('./services/ChartRenderer'),
    import('./lib/discord/client'),
    import('./lib/discord/commands'),
    import('zod'),
    import('child_process'),
    import('util'),
    import('./config/validated'),
    import('./shared/performance/timerManager'),
    import('./shared/performance/RateLimiterManager'),
    import('./shared/enums'),
